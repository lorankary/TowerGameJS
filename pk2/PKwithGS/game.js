...
class Game {
  ...
  constructor() { // from setup()
    ...
    //this.cnv.background(20, 200, 50);

    //select everything of type/class and set call backs
    loadDOMCallBacks(selectAll(".menuTileDiv"));
  ...
  createTower() {
    ...
    //this.towers[this.towers.length-1].visible = true;
...
function handleCNVMouseOver() {
  ...
  if(towerGame.towers[towerGame.towers.length-1].placed === false){
    //towerGame.towers[towerGame.currentTower].loc.x = mouseX;
    //towerGame.towers[towerGame.currentTower].loc.y = mouseY;
  }
}
